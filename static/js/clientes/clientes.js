...
function renderInstalacoesPage() {
    ...
    instalacoesData.slice(start, end).forEach(inst => {
        ...
        tbody.append(`
            <tr>
                <td>${createLink((`/clientes/buscar-instalacao-listagem/${inst.codigo_instalacao}`), inst.codigo_instalacao)}</td>
                <td>${inst.endereco || ''}</td>
                <td>${inst.bairro || ''}</td>
                <td>${statusBadge}</td>
            </tr>
...
function buscarProximoCliente() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (!data || !data.sequencia) {
                Swal.fire({
                    icon: 'info',
                    title: 'Aviso',
                    text: 'Nenhum próximo cliente encontrado.',
                    toast: true,
                    position: 'top-end',
                    timer: 3000,
                    showConfirmButton: false
        ...
        error: function() {
            Swal.fire({
                icon: 'error',
                title: 'Erro ao buscar próximo cliente',
